...
第一方适配器不会把解析器为了维持数组形状而选择的第一列自动当成已确认 X 轴。只有解析器
明确恢复了该轴，并且 Origin 元数据将它指定为 `X`，或列名/单位具有明确的波长语义
（例如 `Wavelength`、`波长`、`λ` 或 `nm`）时，worksheet 才能进入后续 PL 列检测。
`Time`、`Fit`、行号、能量、温度等明确表示其他物理量的轴不会被解释成 nm。解析器返回的
designation 原样保留；缺失 designation 时不会由适配器伪造 `X`。当 workbook 的显示长名称
与 Origin 短名称不同时，来源名采用 `Long [Short]`，避免多个 workbook 因显示名称相同而
失去可追溯性。
...
当前仓库运行的是代码生成的最小 CPYA 合成回归样本，用于在 vendored parser 层验证旧格式
OPJ 的容器解码、工作簿枚举和失败边界。该最小容器不包含可确认的 Origin X designation、
列长名称或 nm 单位，因此第一方适配器会按设计拒绝把其中第一列猜成波长；测试不把它作为
应用层成功导入的依据。它不是公开真实 OPJ fixture，也不是 PL 仪器数据的科学验收证据。
...
- 旧 OPJ 中未被当前解析器解码为 float64 的 float32、整数、text-numeric 等列；这些列不会
  被静默替代为其他列，包含它们的项目需要用可再分发 fixture 单独验证；
- 旧 OPJ 没有端到端完整性校验；某些尾部截断文件可能只恢复前段 workbook 而不报错，使用者
  必须将 workbook/worksheet 数量、列数、点数和波长范围与仪器记录核对；
